...
;(function($) {
  ...
  $.extend($.facybox, {
    ...
    html : function(url){
        ...
        // var url = window.Url2
        // var str = url.split('/')[7].split('.')[0]
        url_split_len = url.split('/').length
        var str = url.split('/')[url_split_len-1].split('.')[0]
        // alert(str)
        // alert(str)
      ...
      return '\
		<div id="facybox"> \
			<div class="popup"> \
				...
				<button class="btn btn-primary" type="submit" style="margin-left: 15px" onclick=location.href=("/image_del\/' + str + '\")>削除</button> \
			</div> \
		</div> \
    ...
    html2 : function(url){
        ...
        // var str = url.split('/')[7].split('.')[0]
        // strs[strs.length-1]
        var str = url.split('/')[url.split('/').length-1].split('.')[0]
        // alert(str)
      ...
      return '\
		<div id="facybox"> \
			<div class="popup"> \
				...
				<button class="btn btn-danger" type="submit" style="margin-left: 15px" onclick=location.href=("/image_del\/' + str + '\")>削除</button> \
			</div> \
		</div> \
		<div class="loading"></div> \
	'
...
// <button class="btn btn-primary" type="submit" style="margin-left: 15px" onclick=location.href=("/test\/' + url + '\';")>削除</button> \
    loading: function(){
      init();
  ...
  function revealImage(href){
    ...
    image.onload = function() {
        // alert(image.src)
		$.facybox.reveal('<div class="image"><img src="' + image.src + '" style="max-height: 500px;"/></div>', $.facybox.content_klass);
    ...
    image.src = href;
    // window.Url2 = image.src
      $('body').append($.facybox.html(image.src));//insert facybox to dom
  }
